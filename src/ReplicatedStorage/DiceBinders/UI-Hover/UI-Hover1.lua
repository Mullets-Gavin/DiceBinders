...
return function(object)
	...
	event1 = object.MouseEnter:Connect(function()
		if not flag then
			flag = true
			local inTween = Services['TweenService']:Create(object,TweenInfo.new(0.1),{Rotation = originalRotation + Intensity()})
			inTween:Play()
		end
	end)
	event2 = object.MouseLeave:Connect(function()
		if flag then
			local outTween = Services['TweenService']:Create(object,TweenInfo.new(0.1),{Rotation = originalRotation})
			outTween:Play()
